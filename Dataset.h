...
public:
    ...
    void TransposeData() {
        transponed_binary_data_ = std::vector<std::vector<bool> >(binary_data_[0].size(), std::vector<bool>(binary_data_.size()));
        for (int i = 0; i < binary_data_.size(); ++i) {
            for (int j = 0; j < binary_data_[i].size(); ++j) {
                transponed_binary_data_[j][i] = binary_data_[i][j];
            }
        }
    }
    
    bool getTransposeElement(int i, int j) const {
        return transponed_binary_data_[i][j];
    }
